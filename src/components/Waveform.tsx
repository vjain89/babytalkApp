...
export default function Waveform({
  ...
  const startBarIndex = Math.max(0, currentBarIndex - Math.floor(barsPerWindow / 2));
  const endBarIndex = Math.min(totalBars, startBarIndex + barsPerWindow);

  const visiblePeaks = peaks.slice(startBarIndex, endBarIndex);
      ...
      <View style={styles.waveformContainer}>
        {visiblePeaks.map((height, i) => {
          ...
          const isTagTick = tagTimestamps.some(
            (t) => Math.abs(t - barTimestamp) < barDurationMs / 2
          );
